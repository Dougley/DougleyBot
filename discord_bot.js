...
bot.on("message", function(msg) {
    var cmd = Commands[command];
			...
			if (!msg.isPrivate) {
				channelInfo = "[$" + msg.channel.server.name + "] [#" + msg.channel.name + "]";
			}
			...
			if (msg.content.charAt(0) === "!") {
				...
				if (antiIdiot.test(command) === false) {
					return;
				}
				...
				if (Commands[command]) {
					if (Commands[command]) {
						var cmdCheckSpec = canProcessCmd(cmd, chunks, msg.author.id, msg);
						...
						bot.reply(msg, "there is no **" + command + "** command.").catch(error);
					}
				}
			}});
